...
# Nothing to do here in the normal case.  fppd loads libfpp-Capture.so once at its own
...
if [ ! -f libfpp-Capture.so ]; then
    echo "Running fpp-Capture PreStart Script - libfpp-Capture.so is missing, rebuilding"
    make "SRCDIR=${SRCDIR}"
fi
